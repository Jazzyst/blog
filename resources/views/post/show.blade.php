...
@section('content')
    <div class="blog-post">
        ...
        </h2>Опубликовано в <a href="/category/{{$post->category->category_id}}">{{$post->category->name}}</a> {{$post->created_at->toFormattedDateString()}} Пользователем {{ $post->user->name ?? "Deleted" }}
